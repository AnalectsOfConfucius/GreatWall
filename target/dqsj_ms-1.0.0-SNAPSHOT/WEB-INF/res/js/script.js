...
//权限
var parentPanel = $('#wrapper',window.parent.document);
 ...
 * @returns {Boolean}
 */
function isAuth(panel,auth){
	var ptop = $('#topMenu',window.parent.document);
	var href = ptop.find('a.active').attr('data-id');
	var cm = parentPanel.find('.childMenu').find('a[href="'+href+'"]');
	var id = cm.attr('data-menuId');
	var opjson = menuOperatJson[id];
	if(!opjson[auth]){
		panel.remove();
//		wxm.dialog({
//            type : 'warning',
//            message : "无添加权限，请联系超级管理员！"
//        });
		return false;
	}
	return true;
