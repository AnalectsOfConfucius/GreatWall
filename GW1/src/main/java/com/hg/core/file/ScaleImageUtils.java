...
public abstract class ScaleImageUtils {
	...
	private static final String DEFAULT_FILE_PATH = "E:/temp-";
	 ...
	 * @throws IOException
	 */
	public static String resize(int width, int height, String fix, String savePath, Image targetImage) throws ImageFormatException, IOException {
		...
		if (savePath == null || "".equals(savePath)) {
			savePath = DEFAULT_FILE_PATH + System.currentTimeMillis() + fix;
		}
	 ...
	 * @throws IOException
	 */
	public static String resize(int width, int height, Float quality, String fix, String savePath, Image targetImage) throws ImageFormatException, IOException {
		width = Math.max(width, 1);
		height = Math.max(height, 1);
		BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		image.getGraphics().drawImage(targetImage, 0, 0, width, height, null);
	...
	public static String resize(int width, int height, String savePath, File targetFile) throws ImageFormatException, IOException {
		...
		return resize(size[0], size[1], fix, savePath, image);
	}
	...
	public static String resize(int width, int height, String savePath, URL targetURL) throws ImageFormatException, IOException {
		...
		int[] size = getSize(width, height, image);
		String filename = targetURL.getPath();
		String fix = filename.substring(filename.lastIndexOf(".") + 1);
		return resize(size[0], size[1], fix, savePath, image);
	}
	...
	public static void main(String[] args) throws ImageFormatException, MalformedURLException, IOException {
		...
		// System.out.println(ScaleImageUtils.resize(140, 140,
		// "E:/img/Mypsd_22841_201211021339450010B.jpg", new
		// URL("http://www.open-open.com/lib/images/logo.jpg")));

		ScaleImageUtils.resize(110, 85, "E:/img/Mypsd_22841_201211021339450010B_1.jpg", new File("E:/img/Mypsd_22841_201211021339450010B.jpg"));
	}
}