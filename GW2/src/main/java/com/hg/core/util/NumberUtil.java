...
public class NumberUtil {
	 ...
	 * @return
	 */
	private static String toResult(char operator, String sOpertand1, String sOpertand2) {
		...
		try {
			while (true) {
				...
				if (isOperator(strExpression.charAt(pos)) && bNegative == false) {
					...
					strValue = "";
					if (!operator.isEmpty()) {
						if (priority(strExpression.charAt(pos)) <= priority(((String) operator.lastElement()).charAt(0))) {
							...
							if (!operator.isEmpty() && priority(strExpression.charAt(pos)) == 1) {
								strOpertand1 = (String) operation.pop();
								strOpertand2 = (String) operation.pop();
								curOper = ((String) operator.pop()).charAt(0);
								operation.push(toResult(curOper, strOpertand2, strOpertand1));
					...
					strValue = procSum(strExpression.substring(pos + 1, pos + rightPos + 1));
					pos += rightPos + 2;
					...
					// 处理非法字符
					if (strExpression.charAt(pos) != ')' && (!strExpression.substring(pos, pos + 1).equals(" "))) {
	 ...
	 * @return 小数部分
	 */
	public static int getDecimal(Double dbl, int scale) {
		if (dbl == null) {
			return 0;
		}
		if (scale < 0) {
		...
		// 转换为字符串形式
		String strVal = dbl.toString();
		// 获取到小数部分
		strVal = strVal.substring(strVal.indexOf('.') + 1, strVal.length());
		if (scale > strVal.length()) {
			scale = strVal.length();
		}
		return strToInt(strVal.substring(0, scale));
	}
